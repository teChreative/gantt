...
//   createTaskEl(taskDataObj, left, width1);
// };
...
var createTaskEl = function (taskDataObj) {
  ...
  const myTask = taskDataObj.task;
  alert(myTask);
  // code from createChart function + + + + + + + START
    //myTask.forEach(el => {
      const duration = taskDataObj//.dataset.duration.split("-");
      const startDay = duration[0];
      const endDay = duration[1];
      let left = 0, width = 0;
    ...
    //});
  // Code from createChart function + + + + + + + END
  ...
  listItemEl.innerHTML = `<li data-duration="${taskDataObj.start}-${taskDataObj.end}" data-color="#b03532" class="task-item" style="left: ${left}px; width: ${width1}px; background-color: rgb(51, 168, 165); opacity: 1;">${taskDataObj.task}</li>`;
